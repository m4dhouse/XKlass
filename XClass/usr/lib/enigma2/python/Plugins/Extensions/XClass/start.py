...
class XClass_Start(Screen):
    ...
    def check_dependencies(self):
        ...
        if dependencies is False:
            os.chmod("/usr/lib/enigma2/python/Plugins/Extensions/XClass/dependencies.sh", 0o0755)
            cmd1 = ". /usr/lib/enigma2/python/Plugins/Extensions/XClass/dependencies.sh"
    ...
    def addServer(self):
        from . import server
        self.session.openWithCallback(self.exit, server.XClass_AddServer)
    ...
    def openCategories(self):
        if self.lastcategory:
            if self.lastcategory == "live":
                from . import live
                self.session.openWithCallback(self.exit, live.XClass_Categories)
            elif self.lastcategory == "vod":
                from . import vod
                self.session.openWithCallback(self.exit, vod.XClass_Categories)
            elif self.lastcategory == "series":
                from . import series
                self.session.openWithCallback(self.exit, series.XClass_Categories)
            elif self.lastcategory == "catchup":
                from . import catchup
                self.session.openWithCallback(self.exit, catchup.XClass_Categories)
        else:
            from . import live
            self.session.openWithCallback(self.exit, live.XClass_Categories)
