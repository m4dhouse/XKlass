...
with open("/usr/lib/enigma2/python/Plugins/Extensions/XClass/version.txt", "r") as f:
    version = f.readline()

screenwidth = getDesktop(0).size()

dir_etc = "/etc/enigma2/xclass/"
dir_tmp = "/tmp/xclass/"
dir_plugins = "/usr/lib/enigma2/python/Plugins/Extensions/XClass/"
...
config.plugins.XClass = ConfigSubsection()
cfg = config.plugins.XClass
...
cfg.epglocation = ConfigDirectory(default="/etc/enigma2/xclass/epg/")
...
cfg.skin = ConfigSelection(default="xclass", choices=folders)
...
# hdr = {"User-Agent": "Enigma2 - XClass Plugin"}
...
def main(session, **kwargs):
    from . import start
    session.open(start.XClass_Start)
    return
...
class AutoStartTimer:
    ...
    def runUpdate(self):
        print("\n *********** Updating XClass EPG ************ \n")
        from . import update
        update.XClass_Update(self.session)
...
class StartDelay:
    ...
    def query(self):
        ...
        glb_session.open(playlists.XClass_Playlists)
        return
        """
...
def Plugins(**kwargs):
    addFont(os.path.join(font_folder, "m-plus-rounded-1c-regular.ttf"), "xclassregular", 100, 0)
