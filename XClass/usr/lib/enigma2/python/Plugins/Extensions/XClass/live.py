...
class XClass_Categories(Screen):
    ...
    def xmltvCheckData(self):
        ...
        filename = "xclass" + ".sources.xml"
        sourcepath = os.path.join(filepath, filename)
        epgfilename = "xclass." + str(safeName) + ".channels.xml"
                ...
                if self.list2:
                    ...
                    if self.session.nav.getCurrentlyPlayingServiceReference():
                        ...
                        if self.session.nav.getCurrentlyPlayingServiceReference().toString() != self.reference.toString() and cfg.livepreview.value is True:
                            ...
                            self.session.openWithCallback(self.setIndex, streamplayer.XClass_StreamPlayer, str(next_url), str(streamtype), str(direct_source), stream_id)
                    else:
                        self.session.openWithCallback(self.setIndex, streamplayer.XClass_StreamPlayer, str(next_url), str(streamtype), str(direct_source), stream_id)
    ...
    def showHiddenList(self):
        ...
        if self["key_menu"].getText() != "":
            ...
            if self["main_list"].getCurrent():
                if self.level == 1:
                    self.session.openWithCallback(self.createSetup, hidden.XClass_HiddenCategories, "live", self.prelist + self.list1, self.level)
                elif self.level == 2 and self.chosen_category != "favourites" and self.chosen_category != "recents":
                    self.session.openWithCallback(self.createSetup, hidden.XClass_HiddenCategories, "live", self.list2, self.level)
    ...
    def buildXMLTV(self):
        ...
        if not os.path.isfile(sourcefile) or os.stat(sourcefile).st_size == 0:
            with open(sourcefile, "w") as f:
                ...
                xml_str += '<sources>\n'
                xml_str += '<sourcecat sourcecatname="XClass EPG">\n'
                xml_str += '</sourcecat>\n'
                xml_str += '</sources>\n'
                f.write(xml_str)
