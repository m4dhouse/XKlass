...
class XClass_DownloadManager(Screen):
    ...
    def stopDownloads(self):
        # Stop all active tasks related to XClass
        for job in JobManager.getPendingJobs():
            if "XClass" in job.cmdline:
    ...
    def resumeDownloads(self):
        for video in self.downloads_all:
            ...
            if state != "Not Started":
                ...
                cmd = "wget -U 'Enigma2 - XClass Plugin' -c '{0}' -O '{1}'".format(self.url, os.path.join(shortpath, filename))
                if "https" in url:
                    cmd = "wget --no-check-certificate -U 'Enigma2 - XClass Plugin' -c '{0}' -O '{1}'".format(self.url, os.path.join(shortpath, filename))
    ...
    def download(self):
        ...
        if current_item:
            ...
            if current_item[3] == _("Not Started"):
                ...
                cmd = "wget -U 'Enigma2 - XClass Plugin' -c '{0}' -O '{1}'".format(self.url, os.path.join(self.shortpath, filename))
                ...
                if "https" in str(self.url):
                    ...
                    if pythonVer == 2:
                        ...
                        if result == 0:
                            cmd = "wget --no-check-certificate -U 'Enigma2 - XClass Plugin' -c '{0}' -O '{1}'".format(self.url, os.path.join(self.shortpath, filename))
                        else:
                            if isDreambox:
