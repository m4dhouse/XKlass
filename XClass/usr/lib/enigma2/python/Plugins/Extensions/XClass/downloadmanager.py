...
class XClass_DownloadManager(Screen):
    ...
    def resumeDownloads(self):
        for video in self.downloads_all:
            ...
            if state != "Not Started":
                ...
                cmd = "wget -U 'Enigma2 - XClass Plugin' -c '%s' -O '%s%s'" % (url, shortpath, filename)

                if "https" in str(url):
                    cmd = "wget --no-check-certificate -U 'Enigma2 - XClass Plugin' -c '%s' -O '%s%s'" % (url, shortpath, filename)
    ...
    def download(self):
        if not os.path.exists(cfg.downloadlocation.value) or cfg.downloadlocation.value is None:
            self.session.open(MessageBox, _("Vod Download folder location does not exist.\n\n" + str(cfg.downloadlocation.value) + _("Please set download folder in Global Settings.")), type=MessageBox.TYPE_WARNING)
            return
        ...
        if self["downloadlist"].getCurrent():
            ...
            if self["downloadlist"].getCurrent()[3] == _("Not Started"):
                ...
                cmd = "wget -U 'Enigma2 - XClass Plugin' -c '%s' -O '%s%s'" % (self.url, self.shortpath, filename)
                ...
                if "https" in str(self.url):
                    ...
                    if pythonVer == 2:
                        ...
                        if result == 0:
                            cmd = "wget --no-check-certificate -U 'Enigma2 - XClass Plugin' -c '%s' -O '%s%s'" % (self.url, self.shortpath, filename)
                        else:
