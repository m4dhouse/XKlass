...
class XClass_Categories(Screen):
    ...
    def showHiddenList(self):
        ...
        if self["key_menu"].getText() != "":
            ...
            if self["main_list"].getCurrent():
                if self.level == 1:
                    self.session.openWithCallback(self.createSetup, hidden.XClass_HiddenCategories, "catchup", self.prelist + self.list1, self.level)
                elif self.level == 2:
                    self.session.openWithCallback(self.createSetup, hidden.XClass_HiddenCategories, "catchup", self.list2, self.level)
    ...
    def playCatchup(self):
        ...
        if next_url != "None" and "/live/" in next_url:
            ...
            self.session.openWithCallback(self.setIndex, streamplayer.XClass_CatchupPlayer, str(playurl), str(streamtype))
        else:
            from Screens.MessageBox import MessageBox
