...
class XKlass_MainMenu(Screen):
    ...
    def playlists(self):
        ...
        self.session.openWithCallback(lambda: self.start, playlists.XKlass_Playlists)

    def settings(self):
        from . import settings
        self.session.openWithCallback(lambda: self.start, settings.XKlass_Settings)

    def addServer(self):
        from . import server
        self.session.openWithCallback(lambda: self.start, server.XKlass_AddServer)

    def downloadManager(self):
        from . import downloadmanager
        self.session.openWithCallback(lambda: self.start, downloadmanager.XKlass_DownloadManager)
