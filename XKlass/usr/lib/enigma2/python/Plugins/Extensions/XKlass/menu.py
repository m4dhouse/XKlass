...
class XKlass_Menu(Screen):
    ...
    def createSetup(self):
        ...
        if not self.list:
            self.session.openWithCallback(self.close, MessageBox, (_("No data, blocked or playlist not compatible with XKlass plugin.")), MessageBox.TYPE_WARNING, timeout=5)
    ...
    def __next__(self):
        ...
        if current_item:
            ...
            if category == 0:
                from . import live
                self.session.open(live.XKlass_Categories)
            elif category == 1:
                from . import vod
                self.session.open(vod.XKlass_Categories)
            elif category == 2:
                from . import series
                self.session.open(series.XKlass_Categories)
            elif category == 3:
                from . import catchup
                self.session.open(catchup.XKlass_Categories)
            elif category == 4:
                self.settings()
            elif category == 5:
                self.updateEPG()
    ...
    def settings(self):
        ...
        self.session.openWithCallback(self.start, playsettings.XKlass_Settings)
